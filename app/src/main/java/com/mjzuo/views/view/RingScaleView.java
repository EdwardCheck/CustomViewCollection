...
public class RingScaleView extends View {
    ...
    // 这是每段的颜色值，用来渐变显示进度
    private static final int[] STATE_CORLOR =
            new int[] {0xFF00FF00, 0xFF3299CC, 0xFFD9D919, 0xFFff7f00, 0xFFFF2400, 0xFFFF0000};
    ...
    /** 进度条*/
    int maxProgress;//最大进度范围
    int minProgress;
    int progress;//当前进度
    int beginLocation;//开始滑动的起始位置度数，顶部270 右侧0 底部90 左侧180
    ...
    int inputWordCorlor;
    int inputWorldSpace;
    String currentInputWorldStr;//内容
    ...
    private void init(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        /** 内部圆及圆弧的属性*/
        slideRingCorlor = array.getInt(R.styleable.RingScaleView_slideRingCorlor, 0x80c0c0c0);
        ringBgCorlor = array.getInt(R.styleable.RingScaleView_ringBgCorlor, 0x806cbe89);
        radius = array.getInt(R.styleable.RingScaleView_radius, CommentUtils.dip2px(context, 64));
        wordCorlor = array.getInt(R.styleable.RingScaleView_wordCorlor, Color.BLUE);
        wordSize = array.getInt(R.styleable.RingScaleView_wordSize, 18);

        /** 刻度值*/
        maxProgress = array.getInt(R.styleable.RingScaleView_maxProgress, 100);
        minProgress = array.getInt(R.styleable.RingScaleView_minProgress, 0);
        progress = array.getInt(R.styleable.RingScaleView_progress, 0);

        /** 下面是刻度线的属性*/
        scaleLineCount = array.getInt(R.styleable.RingScaleView_scaleLineCount, 100);
        scaleLineLength = array.getInt(R.styleable.RingScaleView_scaleLineLength, CommentUtils.dip2px(context, 10));
        specialScaleLineLength = array.getInt(R.styleable.RingScaleView_specialScaleLineLength, CommentUtils.dip2px(context, 15));
        scaleToRingSpace = array.getInt(R.styleable.RingScaleView_scaleToRingSpace, CommentUtils.dip2px(context, 0));
        scaleLineNormalCorlor = array.getInt(R.styleable.RingScaleView_scaleLineNormalCorlor, 0xFFbebebe);
        scaleLineWidth = array.getInt(R.styleable.RingScaleView_scaleLineWidth, CommentUtils.dip2px(context, 4));
        beginLineCorlor = array.getInt(R.styleable.RingScaleView_beginLineNormalCorlor, 0xFF00009C);
        specialScaleCorlors = STATE_CORLOR;

        /** 外围字需要的属性*/
        outWordCorlor = array.getInt(R.styleable.RingScaleView_outWordColor, 0xFF3399FF);
        outWordSize = array.getInt(R.styleable.RingScaleView_outWordSize, CommentUtils.sp2px(context, 12));
        wordWith = array.getInt(R.styleable.RingScaleView_outWordWidth, CommentUtils.sp2px(context, 50));
        wordHeigh = array.getInt(R.styleable.RingScaleView_outWordHeight, CommentUtils.sp2px(context, 20));

        /** 圆圈内文字的属性*/
        inputWordSize = array.getInt(R.styleable.RingScaleView_inputWordSize, CommentUtils.sp2px(context, 14));
        inputWordCorlor = array.getInt(R.styleable.RingScaleView_inputWordColor, 0xFFffffff);
        inputWorldSpace = array.getInt(R.styleable.RingScaleView_inputWordSpace, CommentUtils.sp2px(context, 5));
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        /** 背景圆*/
        canvas.drawArc(new RectF(getPaddingLeft() + specialScaleLineLength + scaleToRingSpace + wordWith
                        , getPaddingTop() + specialScaleLineLength + scaleToRingSpace + wordHeigh
                        , 2 * radius + getPaddingLeft() + specialScaleLineLength + scaleToRingSpace + wordWith
                        , 2 * radius + getPaddingTop() + specialScaleLineLength + scaleToRingSpace + wordHeigh)
                , beginLocation, 360, false, CircleBgPaint);
        ...
        canvas.drawArc(new RectF(getPaddingLeft() + specialScaleLineLength + scaleToRingSpace + wordWith
                        ...
                        , 2 * radius + getPaddingLeft() + specialScaleLineLength + scaleToRingSpace + wordWith
                        , 2 * radius + getPaddingTop() + specialScaleLineLength + scaleToRingSpace + wordHeigh)
                , -90, progress * (360 / 100f), true, RingPaint);

        /** 刻度线*/
        paintScale(canvas);
        /** 外围文字*/
        paintOutWord(canvas, state);

        /** 内部文字*/
        if(currentInputWorldStr != null){
            inputWordPaint.getTextBounds(currentInputWorldStr, 0, currentInputWorldStr.length() ,rect);
            canvas.drawText(currentInputWorldStr
                    , radius + getPaddingLeft() + specialScaleLineLength + scaleToRingSpace + wordWith - (int)(rect.width() * 0.5)
                    , radius + getPaddingTop() + specialScaleLineLength + scaleToRingSpace + wordHeigh + (int)(rect.height() * 0.5  - inputWorldSpace)
                    , inputWordPaint);
        }
    }
    ...
    private void paintScale(Canvas canvas) {
        ...
        // 将坐标系移到圆中心
        canvas.translate(radius + getPaddingLeft() + specialScaleLineLength + scaleToRingSpace + wordWith
        ...
        for(int i = 0; i < scaleLineCount + 1; i ++) {
            ...
            // 每一段的颜色也不同
                ...
                color = CommentUtils.evaluateColor(i, specialScaleCorlors);
            }
            // 特殊方位的线长短
    ...
    private void paintOutWord(Canvas canvas, String state) {
        PointF progressPoint = CommentUtils
                .calcArcEndPointXY(radius + getPaddingLeft() + specialScaleLineLength + scaleToRingSpace + wordWith
                        , radius + getPaddingTop() + specialScaleLineLength + scaleToRingSpace + wordHeigh
